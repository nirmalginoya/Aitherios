        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-[#222]">
              {isLoading ? (
                <tr>
                  <td colSpan="7" className="text-center py-8 text-gray-400">Loading products...</td>
                </tr>
              ) : isError ? (
                <tr>
                  <td colSpan="7" className="text-center py-8 text-red-500">Failed to load products.</td>
                </tr>
              ) : products.length === 0 ? (
                <tr>
                  ...
                  <tr key={item.id} className="hover:bg-[#1a1a1a] transition-colors">
                    ...
                    <td className="px-4 py-4 flex items-center gap-3">
                      {item.images && item.images.length > 0 ? (
                        <img src={item.images[0]} alt={item.name} className="w-10 h-10 rounded-md shrink-0 object-cover" />
                      ) : (
                    ...
                    <td className="px-4 py-4 text-gray-400">{item.category || "Apparel"}</td>
                    <td className="px-4 py-4 font-medium">${item.price?.toFixed(2) || "0.00"}</td>
                    <td className="px-4 py-4">
                      <span className={item.stock === 0 ? "text-red-400" : "text-gray-300"}>
                        {item.stock === 0 ? "Out of Stock" : `${item.stock || 0} in stock`}
                      </span>
                    </td>
                    <td className="px-4 py-4">
